...
产物本身是唯一完成依据：昨日总结为空或仍是默认占位、今日信息简报文件缺失、最近已闭合自然周或自然月有记录但对应自动报告文件缺失，都会从头生成。状态文件不保存任务完成进度；旧版完成游标会被清除。`/status` 显示四项真实产物、当前阶段、失败类别和下次重试时间。首次同时补做多项任务时可能持续数分钟。
...
- **DNS、连接、超时、HTTP 408 或服务端 5xx**：单次请求先按 1 秒、2 秒间隔最多尝试三次；仍失败则整项任务从头等待 5 分钟后再试。只要当前失败仍属于网络错误，就继续按 5 分钟间隔；一旦变成模型输出、JSON、校验或其他非网络错误，改为等到下一个整点。
- **其他失败**：不修复模型 JSON，也不做额外改写调用；本次任务直接失败，最早在下一个整点从头重试。正常失败会写入时间门槛，分钟调度不会在当前小时持续消耗 API。
- **手动补跑**：在报告模式执行 `/retry`，全量重试当前失败的日总结、信息简报、自动周报和自动月报。

每次自动尝试都重新读取本地产物并从头执行，不恢复 Agent 中间节点。报告模式 `/retry` 忽略自动等待期限，立即在独立后台进程中重试全部当前失败任务；产物仍标记为自动生成。
