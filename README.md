...
想写什么就写什么——日常、计划、经历、观点、判断、问题或一闪而过的念头。AgentRecord 不要求用户先分类，也不把记录过程变成与 AI 对话。它与写在纸上的核心区别是：每条内容都以统一格式带时间保存，可以通过同样稳定的格式引用过去的日记和报告；当一天、一周、一个月结束后，Agent 在后台整理已经积累的材料，提取其中的观点、思维模型、判断方式、方法论和点子，寻找关联、变化、矛盾与盲点，并沿少量有依据的方向继续探索。

用户负责自由记录与最终判断，Agent 负责在记录之后收集上下文、展开分析和汇报。分析由 Extractor、Cluster、Explorer、World、Reviewer 和 Report 六类职责明确的 Agent 完成；中控统一提供最小上下文和工具权限，Agent 不能直接修改日记、数据库或报告文件。World 按需联网：外部资料不仅用于核查事实和寻找反例，也用于发现相关理论、案例、相邻领域和可能的延伸方向。报告必须区分用户原意、AI 推断与外部材料，其目标不是替用户得出结论，而是拓宽下一轮思考可以看见的范围。这个“低摩擦记录 → 可追溯引用 → 提取思想 → 外部查证与延伸 → 周期报告”的循环，就是 AgentRecord 的核心。

为避免模型转录 32 位持久节点 ID 时产生单字符错误，所有 Agent 调用只使用 `K001`、`T001`、`N001` 等当次调用短别名；中控负责与 SQLite 持久 ID 双向映射。
...
报告模式不接收普通文字，避免把报告操作或临时输入误写入日记。`/a weekly` 和 `/a monthly` 未给日期时，会按日记文件列出最近自然周或自然月，同时显示周期是否闭合、有记录天数和手动/自动报告状态。也可直接指定月份，例如 `/a monthly 2026-06`。同一周期再次生成手动报告时，程序会先确认是否覆盖原手动报告。手动报告启动后会转入后台并自动返回记录模式；当前窗口可继续记录，完成后会显示通知。任务完成前不要关闭该窗口。`/f` 的反馈只影响以后分析，不改写已有 Markdown 报告。
