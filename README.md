...
安装后，操作系统会在登录/重启时以及每分钟启动一次短进程检查。检查不是每分钟都调用模型：任务失败后会记录下一个整点为最早自动重试时间；整点处于锁屏或休眠状态时，解锁或唤醒后的下一分钟补跑。补跑再次失败后等待再下一个整点，不会在该小时内每分钟消耗调用。报告模式 `/retry` 可以不等待整点，立即全量重试当前失败任务。
...
- **短暂 DNS、连接或超时错误**：单次请求按 1 秒、2 秒间隔最多重试三次；任务最终失败时保留失败状态，不推进成功位置，并等待下一个整点。
