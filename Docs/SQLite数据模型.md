...
版本为 0 时直接创建 v3。版本为 3 时直接使用。其他版本不做字段级兼容迁移：

1. 使用 SQLite backup API 生成 `.analysis.sqlite3.v<旧版本>.legacy.bak`；
2. 关闭连接；
3. 移除旧主库、`-wal` 和 `-shm`；
4. 创建全新 v3 数据库。

已有同名备份不会覆盖。该策略符合本次产品重定向“不兼容旧分析结构”的决定，但只作用于派生数据库，绝不删除或改写 `Records/`。

旧备份可能包含用户过去的反馈和派生分析，不能提交到版本库。若需要保留其业务价值，应在人工检查后单独导出，不应重新引入旧 schema 兼容代码。
